...
A rewriting-based system evaluates rewritten plans over a value universe
that contains, next to the regular values and the provenance identifiers,
the aggregate tokens produced by an aggregate-token constructor gate; a
comparison gate then reads a token and produces the predicate provenance
of an aggregate comparison. The formal counterpart is the evaluator
