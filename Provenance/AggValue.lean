...
read off the token); there is no recursion: the kind discipline forbids
aggregation, grouping and ordering *over* aggregate values, and likewise
...
* `collapse` – the deterministic reading: aggregate the whole sequence.
  This is the value a system displays for an uncompared aggregate (the
  actual-world value), and the reading through which
