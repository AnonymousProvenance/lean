...
violates `delta_absorb` – `Why` is not absorptive – so `δ` collapses group
existence to a bare “exists”. -/
...
the instance takes the support indicator rather than the witness-preserving
identity. -/
