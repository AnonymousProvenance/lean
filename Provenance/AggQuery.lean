...
* `Dedup` and `Diff` exist only at all-regular kind vectors – no
  deduplication or difference over token columns;
...
* `Gamma` outputs rows with `base = 𝟙` and the group's factor pending –
  an uncompared group row finalizes to `δ(⊕ U)`, the terminal-aggregation
  annotation of [Sen et al.][sen2026provsql];
* a selection with aggregate atoms multiplies the predicate provenance
  ...
  supersedes nothing. A δ is thus dropped only when its ⊕-operands are
  exactly the compared aggregates' occurrence tokens and the predicate
  entails existence.
...
* a second selection comparing the same group's tokens finds no pending
  ...
  `⊗`-product of their predicate provenances (the `⊗` of the comparison
  gates in a rewritten plan) – which coincides with the joint
  possible-world reading in idempotent semirings;
...
with comparison-operator complementation, exactly as in `HavingPred`. A
selection whose predicate contains *no* aggregate atom
...
real possible world) is out of scope: `Gamma` is the grouped operator
only.
-/
...
finalizes to `δ(⊕ U)` – the terminal-aggregation annotation of a plain
`GROUP BY` output row. -/
...
inductive TermG (T : Type) {n : ℕ} (κ : Fin n → ColKind) where
  ...
  /-- The aggregate-comparison gate: the predicate provenance of
  comparing the token in column `k` against the
...
atoms complement their comparison operator. -/
...
disjunct does. The supersede of the group-existence factor is licensed
...
inductive AggQuery (T : Type) : (n : ℕ) → (Fin n → ColKind) → Type where
  ...
  into a single `prov` output column – the abstract counterpart of the
  `⊕`-gate creation in rewritten plans. -/
  ...
  carry provenance – the typing act of casting a value column to a
  provenance (`uuid`) type. Token columns cannot be retagged. -/
  | Retag : {n : ℕ} → {κ κ' : Fin n → ColKind} →
      (h : ∀ k, (κ k).base = (κ' k).base) → AggQuery T n κ →
      AggQuery T n κ'
  /-- Token-building grouping (the aggregate-token constructor): group by the
  ...
  | _, _, Sel φ q, d =>
    ...
    if φ.hasAggAtom then
      r.map (fun r =>
        ...
        -- carries the factor's annotation list (a δ is dropped only when its
        -- ⊕-operands are exactly the compared aggregates' occurrence
        -- tokens; a multi-group predicate keeps every group factor)
...
classically-failing rows with annotation `𝟘`, so adequacy is stated on
the query stripped of its aggregate
...
The data-part adequacy of `evaluateAnnotated`
