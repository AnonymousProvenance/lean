...
* an *equality with the stripped query* (`Query.evaluateAnnotated_toPlain`):
  ...
  only rewrites annotations (possibly to `0`), faithfully to
  [Sen et al.][sen2026provsql], where rows are never deleted by difference
...
* [Sen et al., *ProvSQL*][sen2026provsql]
-/
