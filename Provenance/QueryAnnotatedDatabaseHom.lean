...
This is the formal counterpart to the “compile once, evaluate many”
architecture of ProvSQL [Sen et al.][sen2026provsql]: each semiring evaluator
is the application of a (different) homomorphism to the same persistent
provenance representation, and this theorem says going through the
homomorphism on the input side and on the output side give the same answer.
...
implementation side, a provenance circuit has gate types that go beyond the
semiring framework altogether: the δ operator for aggregation row
provenance from [Amsterdamer, Deutch & Tannen, *Provenance for aggregate
queries*][amsterdamer2011aggregate], HAVING-style comparison gates, and
the K-semimodule construction for aggregate values. The commutation
