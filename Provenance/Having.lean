...
/-- Include/exclude recurrence for the JOIN-based provenance `S` (Lemma 15
of the paper):
...
/-- Upward expansion bound (Lemma 13 of the paper): in an idempotent
m-semiring, the monomial of any subset `V ⊆ U` is bounded above by the sum
of `T_U(W)` over all supersets `W ⊇ V` inside `U`. The proof is by strong
induction on `(U \ V).card`, using `le_plus_monus` for the inductive step
and the auxiliary `sum_le_of_forall_le` to collapse multiplicities by
idempotence. -/
...
/-- **Algebraic skeleton of Theorem 12(i)** for `HAVING count ≥ C`: in an
