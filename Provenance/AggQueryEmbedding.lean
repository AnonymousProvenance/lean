...
operator of the general syntax, and the contexts the kind discipline
admits around a site are exactly the expressible ones.
`GenCountHavingRewrite` replaces
...
`⊗` distributes over `⊖`. The kind discipline forbids deduplicating,
differencing or re-grouping aggregate values, so the expressible contexts
around a site are exactly those a rewriting-based system admits.
-/
