...
A rewriting-based system such as ProvSQL rewrites whole queries in which
classical blocks, `GROUP BY … HAVING` blocks and bare `GROUP BY` blocks
occur as subqueries. The
...
(`AggQuery.havingPredRew`, the aggregate-exposing shape a rewritten plan
has, for an arbitrary Boolean combination of aggregate comparisons) and
...
rule `AggQuery.dedupRew` is the `ε` rule of [Sen et al.][sen2026provsql]
(group by the data columns, `⊕`-sum the provenance column), proven correct
...
the two gates – the comparison gate for an aggregate atom, the indicator
...
algebra – aggregate atoms to comparison gates, regular atoms to
...
exactly the sense in which a rewriting-based system can be correct. -/
...
term supersedes it: the supersede of the `δ` gate is licensed exactly
when every world the predicate accepts has the group non-empty. -/
...
/-- **The rewritten duplicate elimination**: the `ε` rule of
[Sen et al.][sen2026provsql] – group by the data columns and `⊕`-sum the
provenance column – applied to an
...
rows on the right. The rewriting of [Sen et al.][sen2026provsql] encodes
that missing left outer join as a union of two branches: rows whose data
part is *absent* from
