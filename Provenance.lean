...
The annotated relational algebra formalized here, with difference and
aggregation, and its provenance-aware query rewriting are those of
[Sen et al.][sen2026provsql]; the general (kind-indexed) framework and the
`HAVING` results extend them to aggregate comparisons.
...
provenance values – mirror the regular, aggregate-token and `uuid`
column types of a rewriting-based system such as ProvSQL, so the scope
restrictions on aggregate results (no deduplication, difference or
re-grouping over them) are static typing here, not a formalization
convenience. The classical query layer below remains the
...
- `Provenance.AggQuery` – the general (non-fused) HAVING semantics:
  ...
  values, aggregate tokens, provenance values – enforcing the scope
  conditions
...
- `Provenance.AggQueryEmbedding` – the compositional JOIN rewriting,
  ...
  replacement preserves the general evaluator's rows verbatim; the kind
  discipline forbids deduplication, difference and re-grouping over
  aggregate values, so the expressible contexts around a site are exactly
  those a rewriting-based system admits
...
- `Provenance.AggQueryHavingRewriting` – **the rewritten world's
  ...
  is the aggregate-token constructor (explicit annotation term, group
  guard `δ(⊕ occs)` in the `prov` output column) and the two term gates
  are interpreted by their primitives (`TermG.evalRew`): `TermG.cmpAgg` is
  the comparison gate, read by `AggValue.predProv`, and `TermG.chiGate` is
...
- `Provenance.AggQueryClosure` – **the compositional closure of the three
  ...
  columns, the gates in the provenance column – the shape of a rewritten
  plan) for an *arbitrary* predicate with an aggregate atom,
  ...
  becoming a comparison gate and a regular one a `TermG.chiGate`
  ...
  `AggQuery.dedupRew`/`AggQuery.dedupRew_valid`: the `ε` rule of
  [Sen et al.][sen2026provsql] (group by the data columns, `⊕`-sum the
  provenance column) proven
...
- `Provenance.QueryRewriting` – alternative query evaluation by rewriting plain
  ...
  [Sen et al.][sen2026provsql] on the classical syntax, with
...
- `Provenance.Probability` – intensional probabilistic query evaluation: probability
  ...
  statement of Theorem 12 of [Sen et al.][sen2026provsql] reducing
...
- `Provenance.Circuit` – Boolean circuits with structural predicates and
  ...
  semantics ([Sen et al.][sen2026provsql], Section V-D
  step 1).
...
`Provenance.Probability`), and the data-part adequacy results of
`Provenance.QueryAdequacy`. Conversely, this library
does not treat NULL
